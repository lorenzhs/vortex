...
impl CastReduce for Lz4 {
    fn cast(array: ArrayView<'_, Self>, dtype: &DType) -> VortexResult<Option<ArrayRef>> {
        if !dtype.eq_ignore_nullability(array.dtype()) {
            // Type changes can't be handled in LZ4, need to decode and tweak.
            return Ok(None);
        }
...
mod tests {
    ...
    #[test]
    fn test_cast_lz4_i32_to_i64() {
        let mut ctx = SESSION.create_execution_ctx();
        let values = PrimitiveArray::from_iter([1i32, 2, 3, 4, 5]);
        let lz4 = Lz4::from_primitive(&values, 0, &mut ctx).unwrap();
    ...
    #[test]
    fn test_cast_lz4_nullability_change() {
        let mut ctx = SESSION.create_execution_ctx();
    ...
    #[test]
    fn test_cast_sliced_lz4_nullable_to_nonnullable() {
        let mut ctx = SESSION.create_execution_ctx();
        let values = PrimitiveArray::new(
            buffer![10u32, 20, 30, 40, 50, 60],
            Validity::from_iter([true, true, true, true, true, true]),
        );
        let lz4 = Lz4::from_primitive(&values, 128, &mut ctx).unwrap();
        let sliced = lz4.slice(1..5).unwrap();
        let casted = sliced
    ...
    #[test]
    fn test_cast_sliced_lz4_part_valid_to_nonnullable() {
        let mut ctx = SESSION.create_execution_ctx();
    ...
    fn test_cast_lz4_conformance(#[case] values: PrimitiveArray) {
        let lz4 = Lz4::from_primitive(&values, 0, &mut SESSION.create_execution_ctx()).unwrap();
        test_cast_conformance(&lz4.into_array(), &mut SESSION.create_execution_ctx());
    }
}
